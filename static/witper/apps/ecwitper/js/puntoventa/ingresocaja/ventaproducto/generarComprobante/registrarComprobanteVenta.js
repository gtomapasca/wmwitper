var clsRegistrarComprobanteVenta = function() {
    
    this.inputs = {
        numDocumentoCli : { id: 'txtNumDocCli' },
        codProducto : { id: 'txtCodProducto' },
        montoPago : { id: 'txtMontoPago' },
        cantidad : { id: 'txtCantidad' }
    };
    ...
    this.botones = {
        ...
        //-----------------------------------------
        btnAgregarProd : { id: 'btnAgregarProd' },
        btnContinuarGen : { id: 'btnContinuarGen' },
        //-----------------------------------------
        btnRegistrarEmitir : { id: 'btnRegistrarEmitir' },
        btnAceptarEmitir : { id: 'btnAceptarEmitir' }
    };

    this.detalleVenta = undefined;
    this.cantDet = undefined;
    this.iniciarForm = function(){
        var refCls = this;
        this.cargarRangoFechaBusqueda();
        this.mostrarComprobanteVentas(null);
        ...
        detalleVenta = [];
        cantDet = 0;
        //console.log(">>> GTPX-iniciarForm-DetalleVenta: " + JSON.stringify(detalleVenta));
        //console.log(">>> GTPX-iniciarForm-cantDet:" + cantDet);
    ...
    this.cargarRangoFechaBusqueda = function(){
        ...
        $("#dpFecIniBusqVta").val(fechaIni);
        $("#dpFecFinBusqVta").val(fechaFin);
        //$("#dpFecIniBusqVta").inputmask();
        //$("#divFecIniBusqVta").datepicker({format: 'DD/MM/YYYY', pickTime: false, language: 'es'});
        $("#dpFecIniBusqVta").datepicker();
	    $("#dpFecFinBusqVta").datepicker();
    };
    ...
    this.onClickBtnCrearComprob = function () {
        ...
        $('#divListarItemsVenta').toggle('slow');

        this.cargarFechaEmisionReg();
    ...
    this.isFiltroBusquedaOK = function(){
        ...
        if($("#dpFecIniBusqVta").val() == '' || $("#dpFecFinBusqVta").val() == ''){
            mostrarMensaje("ingrese rango de fechas", 2);
            return false;			
        }
        //}
        if($("#cboTipComprobBusq").val() != '-1'){
            let sComp = $("#txtSerieComprobBusq").val().trim();
            let nComp = $("#txtNumComprobBusq").val().trim();
            if(sComp != '' || nComp != ''){
                if(sComp == ''){
                    mostrarMensaje("ingrese serie de comprobante", 2);
                    return false;			
                }else if(nComp == ''){
                    mostrarMensaje("ingrese número de comprobante", 2);
                    return false;			
                }
            }
        }
        if($("#cboTipDocBusq").val() != '-1' && $("#cboTipDocBusq").val() != '01'){
            if($("#txtNumDocBusq").val().trim() == ''){
                mostrarMensaje("ingrese número de documento", 2);
                return false;			
            }
        }
        return true;
    ...
    this.aceptarBusq = function(){
        if(this.isFiltroBusquedaOK()){
            let filtro = {};
            let existeUnFiltro = false;
            //if($("#cboTipoRangoFecha").val() != '-1'){
            filtro["fecIniBusq"] = $("#dpFecIniBusqVta").val();
            filtro["fecFinBusq"] = $("#dpFecFinBusqVta").val();
            existeUnFiltro = true;
            //}
            if($("#cboCodEstadoBusq").val() != '-1'){
                filtro["estado"] = $("#cboCodEstadoBusq").val();
                existeUnFiltro = true;			
            }
            if($("#cboTipComprobBusq").val() != '-1'){
                filtro["cod_tipcom"] = $("#cboTipComprobBusq").val();
                existeUnFiltro = true;			
            }
            if($("#txtSerieComprobBusq").val().trim() != ''){
                filtro["nro_serie"] = $("#txtSerieComprobBusq").val();
                existeUnFiltro = true;			
            }
            if($("#txtNumComprobBusq").val().trim() != ''){
                filtro["nro_correlativo"] = $("#txtNumComprobBusq").val();
                existeUnFiltro = true;			
            }
            if($("#cboTipDocBusq").val() != '-1'){
                filtro["cod_tipdoc"] = $("#cboTipDocBusq").val();
                filtro["num_docume"] = $("#txtNumDocBusq").val();
                existeUnFiltro = true;
            }
            //console.log(">>> GTP-Comprobante-filtro: " + JSON.stringify(filtro));
            if(existeUnFiltro){
                this.consultarComprobanteVentas(filtro);	
            }else{
    ...
    this.consultarComprobanteVentas = function(datos){
        let refCls = this;
        witper_consultarComprobanteVentas(datos, function(errorLanzado, datosDevuelto){
            if(errorLanzado == null){
                ...
                if(exito){
                    if(datos != 0){
                        refCls.mostrarComprobanteVentas(datos);
                    }else{
                        //limpiarBusq();
                        mostrarMensaje("No se obtuvieron resultados de la última búsqueda", 2);
                        refCls.mostrarComprobanteVentas("");
                    }
                }else{
    ...
    let TABLE_CONF_RESULT_BUSQ_COMPROBANTES =   {
        "language": IDIOMA,
        "searching": false, 
        "ordering" : true, 
        "scrollY": "150px",
    ...
    // 20230225 degui: mostrar datos de comprobantes venta
    this.mostrarComprobanteVentas = function(datos){
        let refCls = this;
        $("div#divResultadoBusqComprobantes").empty();
        var htmlTablaDetDcl = '';   
        var htmlTblHeader = '<table id="tblResultadoBusqComprobantes" style="border: 1px solid #5b616b" width="1050px"><thead><tr>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Item</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Fecha Emisi&oacute;n</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Nro Comprobante</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Nro Documento</th>';
        htmlTblHeader += '<th class="col-sm-2 text-center" >Nombre / Razón Social</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Estado</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Medio Pago</th>';
        htmlTblHeader += '<th class="col-sm-1 text-center" >Importe Total</th>';
        ...
        $("div#divResultadoBusqComprobantes").html(htmlTablaDetDcl);
        $.each(datos, function(i, item){
                htmlTablaDetDcl = "";
                htmlTablaDetDcl += "<tr style='border: 1px solid #5b616b'>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ (i+1) +"</td>";  
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ (item.fecha_emision).substring(0, 10) +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ (item.nro_serie + "-" + item.nro_correlativo) +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ item.num_docume +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-2 text-left'>"+ (item.nom_cli).toUpperCase() +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ refCls.obtenerDescEstadoComprob(item.estado) +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ refCls.obtenerDescMedioPago(item.id_mediopago) +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>"+ item.pago_total +"</td>";
                htmlTablaDetDcl += "<td class='col-sm-1 text-center'>";
                htmlTablaDetDcl += "	<a href='javascript:registrarComprobanteVenta.cargarVistaComprobanteVenta("+JSON.stringify(item)+")'><i class='glyphicon glyphicon-Edit'></i>Aqui</a>";
                htmlTablaDetDcl += "</td>";
                htmlTablaDetDcl += "</tr>";
                $("div#divResultadoBusqComprobantes table#tblResultadoBusqComprobantes tbody").append(htmlTablaDetDcl);
        });
        $('table#tblResultadoBusqComprobantes').DataTable(TABLE_CONF_RESULT_BUSQ_COMPROBANTES);
        $('div#tblResultadoBusqComprobantes_filter input').prop("placeholder", "Filtro por Nombre");        
    };
    ...
    this.mostrarDatosProducto = function(datos){
        ...
        let ruta_total_img = ruta_upload + datos.dir_img + '/' + datos.nom_img;
        console.log(">>> GTPX-ruta_total_img: " + ruta_total_img);
        $("#hdImgProdReg").val(ruta_total_img);
    ...
    this.onClickBtnAgregarProd = function(){        
        ...
        if(this.validarProductoAgregar()){
            ...
            detalleVenta[cantDet++] = item;
            //console.log(">>>GTPX-onClickBtnAgregarProd-detalleVenta: "+ JSON.stringify(detalleVenta));
            //console.log(">>>GTPX-onClickBtnAgregarProd-cantDet: " + cantDet);
            this.cargarListaProductosVenta(detalleVenta);
    ...
    this.validarProductoAgregar = function() {
        ...
        //console.log(">>> GTPX-validarProductoAgregar-DetalleVenta: " + JSON.stringify(detalleVenta));
    ...
    this.onClickBtnContinuarGen = function() {
        ...
        this.cargarListaItemsPreliminar(detalleVenta);
        // Ocultar formularios pre
        $('#divGenerarComprobante').toggle('hide');
        $('#divListarItemsVenta').toggle('hide');
        $('#divPreComprobante').toggle('slow');        
        //limpiar controles generar
        this.limpiarControlesGenerarVenta();
    };

    // 20230614 Limpiar controles generar venta
    this.limpiarControlesGenerarVenta = function(){
        $("#cboTipoComprobante").val("03").change();
        $("#cboUnidadMedidaReg").val("01").change();
        $("#cboTipoMoneda").val("01").change();
        ...
        $('#opSelDniBusq').prop('disabled', false);
        $('#opSelRucBusq').prop('disabled', true);
        $("#cboTipoPago").val("03").change();
        $("#cboTipoReg").val("1").change();
        $("#hdCodUsuCliReg").val("");
        $("#txtNumDocCli").val("");
        $("#txtNombreCli").val("");
        $("#txtMontoTotal").val("");
        ...
        $('#txtPrecioDescuento').prop('disabled', true);
        $("#idChkPrecDescuento").prop("checked", false);
        $("#idChkEstadoEnvio").prop("checked", false);
        $("#idChkObservacion").prop("checked", false);
        $('#divPagoMontoReg').toggle('hide');
        $('#divPagoVueltoReg').toggle('hide');
        this.cargarListaProductosVenta("");
    }
    ...
    this.cargarListaProductosVenta = function(datos){        
        ...
        let total_importeIGV = 0;
        let total_importeSum = 0;
        $('#tblListarItemsVenta').DataTable().clear();
        $('#tblListarItemsVenta').DataTable().destroy();
        $('#tblListarItemsVenta').DataTable({
            responsive: false,
            autoWidth: false,
            paging:   false,
            searching: false,
            ...
            columns: [
                {data: function ( row, type, val, meta ) {
                        var salida = ++i; 
                        total_importe     += parseFloat(row.importe);
                        total_importeIGV  += parseFloat(row.importeIGV);
                        total_importeSum  += parseFloat(row.importeTotal);
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = "<img style='padding:5px; text-align:center;width:100px;' src='"+(uri_store+row.imgProd)+"'/>"; 
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = row.miniCodProd; 
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = row.cant; 
                ...
                {data: function ( row, type, val, meta ) {
                        var salida = refCls.obtenerDescUnidadMedida(row.codUniMedida); 
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = row.descProd;
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = row.valorUnit;
                        return salida;
                }, sClass: 'text-center'},        
                {data: function ( row, type, val, meta ) {
                        var salida = 0;
                        /*if(row.codDescuento == '000'){ // sin descuento
                            salida = row.precProd;
                        }else */
                        if(row.codDescuento == '001'){ // con descuento
                            salida = (row.precProd - row.precioDescto).toFixed(2);
                            //salida = row.precioDescto;
                        }
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        ...
                        if(row.codDescuento == '000'){ // sin descuento
                            salida = row.precProd;
                        }else if(row.codDescuento == '001'){ // con descuento
                            //salida = (row.precProd - row.precioDescto).toFixed(2);
                            salida = row.precioDescto;
                        }
                        return salida;
                ...
                {data: function ( row, type, val, meta ) {
                        var salida = row.importeIGV;
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        var salida = row.importe;
                        return salida;
                }, sClass: 'text-center'},
                {data: function ( row, type, val, meta ) {
                        //var salida = '<a class=\"link\" href="#" onclick="" ><img onclick="JavaScript:eliminarArchivoPrecinto(\''+meta.row+'\',\''+row.nomArchivo+'\');" class="imgEliminar" src="/a/imagenes/delete.png"/></a>';
                        //var salida = "<a href='javascript:cargarDatosParaModificarItem("+JSON.stringify(row)+")'><i class='glyphicon glyphicon-Edit'></i></a>";
                        var salida = "<i class='glyphicon glyphicon-Edit'></i>";
                        return salida;
                }, sClass: 'text-center'}
            ] 
        });
        $("#sSubTotal").html(total_importe.toFixed(2));
        $("#sIGV").html(total_importeIGV.toFixed(2));
        $("#sImporteTotal").html(total_importeSum.toFixed(2));
    ...
    this.armarDataVenta = function(secComprob){
        ...
        this.registrarComprobanteVenta(dataVenta);

        // Limpiar controles preliminar
        $("#hdCodTipComprobPre").val("");
        $("#txtFechaEmiPre").val("");
        $("#hdCodTipDocuPre").val("");
        $("#txtNumDocCliPre").val("");
        $("#txtNombreCliPre").val("");
        $("#hdCodTipoMonedaPre").val("");
        $("#hdCodTipPagoPre").val("");
        $("#hdPagoTotalPre").val("");
        $("#hdPagoMontoPre").val("");
        $("#hdPagoVueltoPre").val("");
    ...
    this.armarDetalleVenta = function(id){
        ...
        // Limpiar grilla Preliminar
        detalleVenta = [];
        cantDet = 0;
        this.cargarListaItemsPreliminar("");
    };
    ...
    this.onClickBtnAceptarEmitir = function(){
        ...
        this.cargarListaItemsEmitida(detalleVenta);
    };
